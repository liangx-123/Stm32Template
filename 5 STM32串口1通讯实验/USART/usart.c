...
void uart_init(u32 bound){
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART1|RCC_APB2Periph_GPIOA, ENABLE);	//使能USART1，GPIOA时钟
	 ...
	 //USART1_TX   PA.9
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9|GPIO_Pin_1; //PA.9
...
void USART1_IRQHandler(void)                    
 {
     ...
     if(USART_GetITStatus(USART1, USART_IT_IDLE) != RESET)
     {
         ...
         //当接收到一个完整的数据帧的时候，需要先关闭DMA，然后将DMA串口接收相关寄存器复位（目的是为了下一帧数据从头开始接收）
       MYDMA_Config1(DMA1_Channel5, (u32)&USART1->DR, (u32)TEXT_TO_SEND,2); 
		...
		MYDMA_Enable(DMA1_Channel5); //开始一次DMA传输！
         ...
         //关闭串口1的接收空闲中断，等待主程序中处理完这一帧数据后重新打开DMA和接收空闲中断
